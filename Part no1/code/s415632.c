	...
	for (i=1; i<sizeMinus; i+=1) {//common subexpression
		...
		for (j=1; j<sizeMinus; j+=1 ) { //common subexpression
			...
			for (l = -1; l <= 1; l++) {  /* interchange loop */
				...
				res += temp3* horiz_operator[l+1][0];
				res2 += temp3* vert_operator[l+1][0];
				
				temp3 = input[temp4]; //common subexpression
				res += temp3* horiz_operator[l+1][1];
				res2 += temp3* vert_operator[l+1][1];
				
				temp3 = input[temp4 + 1]; //common subexpression
				res += temp3* horiz_operator[l+1][2];
				res2 += temp3* vert_operator[l+1][2];
