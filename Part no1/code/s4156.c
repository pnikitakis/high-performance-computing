...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	sizeMinus = SIZE - 1; //common subexpression
	...
	for (i=1; i<sizeMinus; i+=1) {//common subexpression
		...
		for (j=1; j<sizeMinus; j+=1 ) { //common subexpression
			...
			for (l = -1; l <= 1; l++) {  /* interchange loop */
				for (k = -1; k <= 1; k++) {
					res += input[temp + l*SIZE + k] * horiz_operator[l+1][k+1];
				}
			}					
			...
			/* START second convolution2D() for vertical */
			res=0; 
			
			for (l = -1; l <= 1; l++) { /* interchange loop */
				for (k = -1; k <= 1; k++) {
					res += input[temp + l*SIZE + k]* vert_operator[l+1][k+1];
				}
			}	
			
			resY = pow(res,2);
			/* END second convolution2D() for vertical */
